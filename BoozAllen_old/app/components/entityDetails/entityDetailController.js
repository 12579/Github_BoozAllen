...
(function () {
    ...
    .controller('entityDetailCtrl', ["$state", "$scope", "$stateParams", "$window", "$http", "authenticationSvc", "ShareData", "VisDataSet", function ($state, $scope, $stateParams, $window, $http, authenticationSvc, shareData, VisDataSet) {
        ...
        angular.element('.entityCard.active').index() === 0 ? angular.element('.cardNav .cardPrev').hide() : angular.element('.cardNav .cardPrev').show();
        
        var resetCards = function() {
            var items = angular.element('.entityCard'),
                noOtItems = items.length;
            items.length > 0 ? angular.element(items[0]).addClass('active') : null;
            for (var i = 0 ; i < noOtItems; i++) {
                items[i].style[defaultVars.getVendor.js + "Transform"] = '';
        ...
        $scope.cardNext = function () {
            ...
            for (var i = angular.element(current).index() ; i < noOtItems; i++) {
                items[i].style[defaultVars.getVendor.js + "Transform"] = 'translateY(' + y + 'em) scale(' + z + ')';
                y = y - 2;
                z = z - 0.1;
            }
        ...
        $http.get("../../../sampleJson/personObject.json")
            ...
            primaryData["risktype"] = res.data.person.riskDNA.riskDnaScore <= 33.33 ? 'LOW RISK' : res.data.person.riskDNA.riskDnaScore > 33.33 && res.data.person.riskDNA.riskDnaScore <= 66.66 ? 'MEDIUM RISK' : 'HIGH RISK';
        ...
        $scope.reloadEntityDetail = function (eventDate) {
            ...
            $http.get("../../../sampleJson/personObject.json")
              .then(function (res) {
                  var entityPastEventsData = res.data.history.events;
                  var i;
                  for (i = 0; i < entityPastEventsData.length; i++) {
                      if (Number(new Date(entityPastEventsData[i].eventDate)) === eventDate) {
                          var entityPastEvents = entityPastEventsData[i].snapshot;

                          $scope.entityDetail = entityPastEventsData[i].snapshot;
                          $scope.eventDate = entityPastEventsData[i].eventDate;

                          $scope.entityDetailOptions.rowData = [];
                          $scope.entityDetailOptions.api.onNewRows();

                          if (entityPastEvents.eddPersonData != undefined
                              && entityPastEvents.eddPersonData.negativeNewsData != undefined) {

                              var negativeNewsData = entityPastEvents.eddPersonData.negativeNewsData.articles;
                              $scope.agingData = bindAgingPieChart(negativeNewsData);

                              for (i = 0; i < negativeNewsData.length; i++) {
                                  negativeNewsData[i].doa = dateformat(negativeNewsData[i].doa);
                              }
                          ...
                          if (entityPastEvents.cddPersonData != undefined) {

                              if (entityPastEvents.cddPersonData.activityCDD != undefined) {
                                  $scope.accountgridOptions.rowData = entityPastEvents.cddPersonData.activityCDD.accounts;
                              ...
                              if (entityPastEvents.cddPersonData.activityCDD != undefined) {
                                  $scope.watchListGridOptions.rowData = entityPastEvents.cddPersonData.watchlistCDD.watchlistHits;
                                  $scope.watchListGridOptions.api.onNewRows();

                                  $scope.pepHitsGridOptions.rowData = entityPastEvents.cddPersonData.watchlistCDD.pepHits;
                          ...
                          if (res.data != undefined) {
                              ...
                              $scope.TasksGrid.rowData = "";
                              $scope.TasksGrid.api.onNewRows();

                              $scope.NotesGrid.rowData = "";
                              $scope.NotesGrid.api.onNewRows();

                              $scope.DocumentCenterGrid.rowData = "";
                              $scope.DocumentCenterGrid.api.onNewRows();
                          }

                          $scope.cardcount = entityPastEvents.personas == undefined ? 0 : entityPastEvents.personas.length;
                          var cardcount = entityPastEvents.personas == undefined ? 0 : entityPastEvents.personas.length;

                          var jsonObj = [];
                          for (var i = 0; i < cardcount; i++) {
                              var data = entityPastEvents.personas[i];
                          ...
                          var primaryData = {};
                          primaryData["shortname"] = entityPastEvents.personBaseData.firstName.substr(0, 1) + entityPastEvents.personBaseData.lastName.substr(0, 1);
                          primaryData["score"] = entityPastEvents.riskDNA == undefined ? 0 : entityPastEvents.riskDNA.riskDnaScore;
        ...
        var changeLogColumnDefs = [
           {
               headerName: "User", field: "", width: 950 * defaultVars.w / defaultVars.winRef, suppressMenu: 'true', cellClass: 'rag-entity',
               cellRenderer: function (params) {
                   return '<a href title="Click to view entity log data" ng-click="reloadEntityDetail(' + Number(new Date(params.data.eventDate)) +');">'
                       + '<b>' + params.data.snapshot.personBaseData.firstName + ' ' + params.data.snapshot.personBaseData.lastName
                       + '</b></a> - ' + params.data.snapshot.personBaseData.organizationID;
               }            
           },
        ...
        $scope.file_changed = function (element) {
            ...
            $scope.$apply(function (scope) {
                ...
                reader.onload = function (e) {
                    ...
                    $scope.UploaddedFileData = fileData;
                };
        ...
        $scope.DocumentCenterGridUpload = function (fileDesc) {
        ...
        var tasksGridColumnDefs = [
            ...
            {
                headerName: "",
                width: 475 * defaultVars.w / defaultVars.winRef,
                suppressMenu: 'true',
                cellClass: 'rag-entity',
                cellClassRules: {
